...
class PsController extends BaseController {
	...
	public function group(){
		...
		$this->getPage($count, $psize, 'Joel-loader', '配送站管理','Joel-search');
	...
	public function groupSet(){
		...
		$bread=array(
			...
			'2'=>array(
				'name'=>'配送站设置',
				'url'=>$id?U('Cms/Ps/groupSet',array('id'=>$id)):U('Cms/Ps/groupSet')
			)
		);
		$this->assign('breadhtml',$this->getBread($bread));
		//处理POST提交	
	...
	public function xqSet(){
		...
		$bread=array(
			...
			'2'=>array(
				'name'=>'小区设置',
				'url'=>$id?U('Cms/Ps/xqSet',array('id'=>$id)):U('Cms/Ps/xqSet')
			)
		);
		$this->assign('breadhtml',$this->getBread($bread));
		//处理POST提交	
