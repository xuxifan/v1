...
class ZjdController extends BaseController {
	...
	public function dzp(){
		...
		$this->getPage($count, $psize, 'Joel-loader', '砸金蛋设置','Joel-search');
...
//  	$re=M('zjd_prize')->where($map)->delete();
    	if(FALSE!==$re){
			$info['status']=1;
			$info['msg']='删除成功！';
		}else{
...
//	}
	...
	public function sdpjset(){
		...
		//砸金蛋选项
		$pzds=M('zjd')->where(array('status'=>'1'))->select();
		$this->assign('pzds',$pzds);
		
		//奖项设置
		$ma['dzpid']=$pr['dzpid'];
		$ma['store']=array('neq','0');
		$prize=M('zjd_prize')->where($ma)->select();
	...
	//	$ma['uid']=self::$UID;
		$ma['store']=array('neq','0');
		$pzds=M('zjd_prize')->where($ma)->select();
	...
	public function acformSet(){
		...
		$bread=array(
			...
			'1'=>array(
				'name'=>'万能表单列表',
				'url'=>U('Cms/Zjd/acformList')
			),
			'2'=>array(
				'name'=>'万能表单设置',
				'url'=>$id?U('Cms/Zjd/acformSet',array('id'=>$id)):U('Cms/Zjd/acformSet')
			)
		);
		$this->assign('breadhtml',$this->getBread($bread));
		//处理POST提交	
	...
	public function acformDel(){
		...
		$rq=M('zjd_acform_data')->where(array('formid'=>$id))->find();
		if($rq){
			$info['status']=0;
...
//		$this->ajaxReturn($mb);
	...
	public function orderExport() {
		...
		$data=M('zjd_zj')->where($map)->select();
		...
		$title=array('ID','砸金蛋编号','会员ID','昵称','OPENID','奖项名称','真实名称','记录时间','IP','SN' );
