...
//		}
    ...
    public function prize(){
     	...
     	$bread=array(
			...
			'1'=>array(
				'name'=>'奖品设置',
				'url'=>U('Cms/Active/prize')
			)
		);
		$this->assign('breadhtml',$this->getBread($bread));
		// 增加搜索分页
		$act=M("Dzp");
		$active=$act->where(array('status'=>'1'))->select();
		...
		foreach($all_prize as $k=>$v){
			...
			if(!array_key_exists($v['dzpid'],$dzparr)){
				$dzparr[$v['dzpid']]=$dzp->where(array('status'=>'1'))->where("id=".$v['dzpid'])->find();
			}
    ...
    public function prizeset(){
		...
		//奖品的顶部设置
		$dzp=M("Dzp")->where(array('status'=>'1'))->select();
		$p=M("Dzp_prize");
...
//	}
	...
	public function sdpjset(){
		...
		//大转盘选项
		$pzds=M('dzp')->where(array('status'=>'1'))->select();
		$this->assign('pzds',$pzds);
		
		//奖项设置
		$ma['dzpid']=$pr['dzpid'];
		$ma['store']=array('neq','0');
		$prize=M('dzp_prize')->where($ma)->select();
	...
	public function getprize(){
		...
		$ma['dzpid']=$cid;
		$ma['store']=array('neq','0');
		$pzds=M('dzp_prize')->where($ma)->select();
	...
	public function acformSet(){
		...
		$bread=array(
			...
			'1'=>array(
				'name'=>'万能表单列表',
				'url'=>U('Cms/Active/acformList')
			),
			'2'=>array(
				'name'=>'万能表单设置',
				'url'=>$id?U('Cms/Active/acformSet',array('id'=>$id)):U('Cms/Active/acformSet')
			)
		);
		$this->assign('breadhtml',$this->getBread($bread));
		//处理POST提交	
		...
		//活动
		$active=M("Dzp")->where(array('status'=>'1'))->select();
		$this->assign('active',$active);
	...
	public function acformDel(){
		...
		$rq=M('acform_data')->where(array('formid'=>$id))->find();
		if($rq){
			$info['status']=0;
	...
	public function acformdatainfo(){
		...
		$bread=array(
			...
			'2'=>array(
				'name'=>'活动数据列表',
				'url'=>U('Cms/Active/acformdatainfo')
			)
		);
		$this->assign('breadhtml',$this->getBread($bread));
...
//		$this->ajaxReturn($mb);
	...
	public function orderExport() {
		...
		$data=M('dzp_zj')->where($map)->select();
		...
		$title=array('ID','大转盘编号','会员ID','昵称','OPENID','奖项名称','真实名称','记录时间','IP','SN' );
