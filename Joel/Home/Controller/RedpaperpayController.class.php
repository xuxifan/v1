...
class RedpaperpayController extends Controller {
	//Joel全局相关
	...
	public function pay(){
		...
		self::$_opt['openid']=$openid=$_SESSION['WAP']['vip']['openid'];
		...
		if(!$cache){
			$this->error('此订单不存在！',U('/Wap/Redpaper/cards'));
		}
		if($cache['vipopenid']!=$openid){
			$this->error('此订单参数错误！',U('/Wap/Redpaper/cards'));
		}
		if($cache['ispay']){
			$this->error('此订单已支付！请勿重复支付！',U('/Wap/Redpaper/cards'));
		}
		//微信支付封装
		$options['appid']= self::$_wxappid;
		$options['appsecret']= self::$_wxappsecret;
		...
		$url=self::$_url."/redpaperpay.php?".$str;
...
//		$this->assign('jsapi',$jsapi);
	...
	// 支付失败
	public function errorpay($oid){
		$m=M("Shop_order");
		$cache=$m->where(array('oid'=>$oid))->find();
		$cache['status']=0;
		$m->save($cache);
	}
	...
	public function nd(){
		...
		file_put_contents(self::$_logs.'Joel_redpaperpaynd.txt','响应参数:'.date('Y-m-d H:i:s').PHP_EOL.'通知信息111:'.$str.PHP_EOL.PHP_EOL.PHP_EOL,FILE_APPEND);
		//使用通用通知接口
	...
	public function endpay($oid){
		...
		M('Redpaper')->where('id=1')->setInc('sell',$order['payprice']);
		if ($order) {
	...
	public function afterRedpaper($order){
		$m=M("Redpaper");
