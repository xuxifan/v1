...
class JysWxpayController extends Controller {
	//Joel全局相关
	...
	public function pay(){
		...
		$paysdk->setParameter("notify_url",self::$_url.'/Wxpay/nd/');//交易通知地址 
		...
		if($verify_result) {
			...
			if($result == 'success') {
				//判断该笔订单是否在商户网站中已经做过处理
					...
					$url=self::$_url.'/Wap/Shop/jysorderList/sid/'.self::$_opt['sid'];
					header('Location:'.$url);
		    }
		    else {
		      echo "支付失败";//这里永远不会调用
