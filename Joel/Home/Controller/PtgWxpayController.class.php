...
class PtgWxpayController extends Controller {
	//Joel全局相关
	...
	protected static $_isgroup=0;//是否是拼团购
	protected static $_ptgid=0;//是否是拼团购
...
//		}
		...
		if(!$cache){
			if($sid){
				$this->error('此订单不存在！',U('/Wap/Fxshop/index'));
			}else{
				$this->error('此订单不存在！',U('/Wap/Shop/index'));
			}
			
		}
		if($cache['ispay']){
			if($sid){
				$this->error('此订单已支付！请勿重复支付！',U('/Wap/Fxshop/index'));
			}else{
				$this->error('此订单已支付！请勿重复支付！',U('/Wap/Shop/index'));
			}
		...
		if($cache['ptgid']){
			$mptg=M('Ptg_log');
			$ptg_log_data=$mptg->where('id='.$cache['ptgid'])->find();
			// 检查拼团状态
			if($ptg_log_data['status']!=1){
				$this->error('拼团失败！此拼团已结束！',U('/Wap/Shop/ptgorderlist'));die;
			}
			// 检查认识是否已满
			if($ptg_log_data['num']>=$ptg_log_data['groupmax']){
				//人数已满
				$this->error('拼团失败！拼团人数已满！',U('/Wap/Shop/ptgorderlist'));die;
			}
		}
		// 再次检查ptg是否能成功
		...
		$paysdk->setParameter("notify_url",self::$_url.'/PtgWxpay/nd/');//交易通知地址 
		...
		$url=self::$_url."/ptgwxpay.php?".$str;
...
//		$this->assign('jsapi',$jsapi);
	...
	public function cz(){
		...
		$paysdk->setParameter("notify_url",self::$_url.'/PtgWxpay/cznd/');//交易通知地址 
		$paysdk->setParameter("trade_type","JSAPI");//交易类型
		$prepayid=$paysdk->getPrepayId();
		//dump($options);
		//die($prepayid);
...
//		$this->assign('jsapi',$jsapi);
	...
	public function paycancel(){
		...
		if($sid){
			$url=self::$_url.'/Wap/Fxshop/ptgorderlist/sid/'.$sid;
		}else{
			$url=self::$_url.'/Wap/Shop/ptgorderlist/sid/'.$sid;
		}
		$url=self::$_url.'/Wap/Shop/ptgorderlist/sid/'.$_SESSION['wxpaysid'];
		...
		if($verify_result) {
			...
			if($result == 'success') {
				//判断该笔订单是否在商户网站中已经做过处理
					...
					if(self::$_isgroup){
						$url=self::$_url.'/Wap/Ptg/index/ptgid/'.self::$ptgid;
						header('Location:'.$url);
					}else{
						$url=self::$_url.'/Wap/Shop/ptgorderlist/sid/'.self::$_opt['sid'];
						header('Location:'.$url);
					}
		    }
		    else {
			...
			if($order['status'] == 1){
					...
					if($order['isgroup']==2){
						...
						if($order['ptgid']){
							...
							//$order['ptgid']=self::$_ptgid;
							$ptgorder['ptgid']=$order['ptgid'];
							$ptgorder['oid']=$order['oid'];
							$ptgorder['vipid']=$order['vipid'];
							$ptgvip=M('Vip')->where('id='.$order['vipid'])->find();
							$ptgorder['nickname']=$ptgvip['nickname'];
							$ptgorder['openid']=$ptgvip['openid'];
							$ptgorder['headimgurl']=$ptgvip['headimgurl'];
							...
							$ptgdata['vipid']=$order['vipid'];
							$ptgdata['sid']=$order['sid'];
