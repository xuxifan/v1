...
class AlipaywapController extends Controller {
	...
	public function __construct($options){
		...
		$this->alipay_config['partner']		= '2088711812458610';
		//安全检验码，以数字和字母组成的32位字符
		$this->alipay_config['key']			= '6j3ztz4bbsu2zy076272gwvvgz5lsxgf';
		...
		$this->alipay_config['sign_type']    = 'MD5';//Joel修改为MD5验证模式
	...
	public function pay(){
		...
		//卖家支付宝帐户
		$seller_email = 'cjz@wgongchang.com';//$_POST['WIDseller_email'];
	...
	public function paycancel(){
		$url=self::$_url.'/Wap/Shop/orderList/sid/'.$_SESSION['alipaysid'];
	...
	public function nd(){
			...
			//file_put_contents(self::$_logs.'Joel_nd.txt','响应参数:'.date('Y-m-d H:i:s').PHP_EOL.'通知信息:'.$str.PHP_EOL.PHP_EOL.PHP_EOL,FILE_APPEND);
			...
			//测试模式下打印响应信息
			file_put_contents(self::$_logs.'Joel_nd.txt','签名验证成功:'.date('Y-m-d H:i:s').$trade_status.PHP_EOL.'通知信息:'.$str.PHP_EOL.PHP_EOL.PHP_EOL,FILE_APPEND);
	...
	public function cznd(){
		    ...
		    if($trade_status == 'TRADE_FINISHED') {
				...
				$this -> endcz($out_trade_no);
				file_put_contents(self::$_logs.'Joel_nd.txt','会员充值成功:'.date('Y-m-d H:i:s').PHP_EOL.'通知信息:'.$str.PHP_EOL.'订单状态:'.$order.PHP_EOL.'交易类型:TRADE_FINISHED'.PHP_EOL.PHP_EOL,FILE_APPEND);
		        ...
		        // 交易处理成功--此处可以对交易做分润等处理
				$this -> endcz($out_trade_no);
				file_put_contents(self::$_logs.'Joel_nd.txt','会员充值成功:'.date('Y-m-d H:i:s').PHP_EOL.'通知信息:'.$str.PHP_EOL.'订单状态:'.$order.PHP_EOL.'交易类型:TRADE_SUCCESS'.PHP_EOL.PHP_EOL,FILE_APPEND);
		    }
