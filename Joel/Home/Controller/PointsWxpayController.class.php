...
class PointsWxpayController extends Controller {
	//Joel全局相关
	...
	public function pay(){
		...
		self::$_opt['orderid']=$orderid=$_GET['orderid'];
		...
		$paysdk->setParameter("notify_url",self::$_url.'/PointsWxpay/nd/');//交易通知地址 
		...
		$url=self::$_url."/pointswxpay.php?".$str;
...
//		$this->assign('jsapi',$jsapi);
	...
	public function cz(){
		...
		$paysdk->setParameter("notify_url",self::$_url.'/PointsWxpay/cznd/');//交易通知地址 
		$paysdk->setParameter("trade_type","JSAPI");//交易类型
		$prepayid=$paysdk->getPrepayId();
		//dump($options);
		//die($prepayid);
	...
	public function endpay($oid,$buyer_email){
		...
		if ($order) {
			if($order['status'] == 1){
					...
					$vip['score']=$vip['score']-$order['integpay'];
					$reas=$mvip->save($vip);
