...
class YydbWxpayController extends Controller {
	//Joel全局相关
	...
	public function pay(){
		...
		$opt=I('get.');
		//self::$_opt['oid']=$oid=$_GET['oid'];//单个Oid
		self::$_opt['oid']=$oid=M('yydb_log')->where(array('id'=>$_GET['oid']))->getField('oid');
		self::$_opt['oids']=$orderid=$_GET['oids'];//多个订单号
		...
		for($i=0;$i<count($arr);$i++){
			$arrs=$logs->where(array('id'=>$arr[$i]))->find();
			$totalprice+=$arrs['totalprice'];//总价格
			$totalnum+=$arrs['totalnum'];//总数量
			$payprice+=$arrs['payprice'];//支付价格
		}

		//取ORDER
		$cache=$logs->where(array('oid'=>$oid))->find();
		...
		$paysdk->setParameter("notify_url",self::$_url.'/Home/YydbWxpay/nd/orderid/'.$orderid);//交易通知地址 
		...
		$url=self::$_url."/yydbwxpay.php?".$str."goodsid=".$cache['goodsid'];
...
//		$this->assign('jsapi',$jsapi);
	...
	public function payback(){
		...
		$sta='0';
		$msg='';
		$verify_result = $this->verifyReturn();//die;
		if($verify_result) {
			//验证成功
	...
	public function nd(){
		...
		if($notify->checkSign() == TRUE)
		{
			...
			elseif($notify->data["result_code"] == "FAIL"){
				...
				$this -> endpay($out_trade_no,$_GET['orderid']);
	...
	//付款成功后操作
	public function endpay($oid,$orderid){
		$m = M('yydb_log');
		$ygoods=M('yydb_goods');
		$sorder=M('shop_order');
