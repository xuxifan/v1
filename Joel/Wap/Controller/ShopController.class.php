...
class ShopController extends BaseController {
	...
	public function _initialize() {
		...
		//输出商城设置
		$shopset['imgurl']=$shopset['sharepic'];
		$this->assign('indexset',$shopset);
		//追入分享特效
		$options['appid']= self::$_wxappid;
		$options['appsecret']= self::$_wxappsecret;
		$wx = new \Joel\wx\Wechat($options);
		// 作者：郑伊凡 2016-1-21 母版本 功能：用于前台判断是否开启身份验证
 ...
 //首页获取更多
	/* public function indexMore(){
			if(IS_AJAX){						
				...
				$map='';
				$pageSize=5;
				$goodslist=$m->where(array('lid'=>$v['id'].',','status'=>1))->limit(($page-1)*$pageSize+1,$pageSize)->order('sorts desc')->select();
				if($goodslist){
					foreach($goodslist as $k=>$v){
						$ta=$this->getPic($goodslist[$k]['pic']);
						$goodslist[$k]['imgurl']=$ta['imgurl'];
	...
	public function group(){
		...
		if($tmpgroup){
				...
				$tmplist['goods']=$m->where($tmpmap)->order('sorts desc')->select();
				if($tmplist['goods']){
	...
	public function search(){
		...
		$map['iscut']=0;
		$map['isgroup']=0;
		$goodslist=$m->where($map)->order('sorts desc')->select();
		foreach($goodslist as $k=>$v){
			$ta=$this->getPic($goodslist[$k]['pic']);
			$goodslist[$k]['imgurl']=$ta['imgurl'];
		}
	...
	public function cate(){
		...
		$wap['iscut']='0';
		$wap['isgroup']='0';
		$goodslist=$m->where($wap)->order('sorts desc')->select();
//		$next=$m->where('cid='.$id.' and status=1')->page(2,$pagesize)->order('sorts desc')->select();
//		if($next){
...
//		}		
	...
	public function cateMore(){
		if(IS_AJAX){						
			...
			$pagesize=$shopset['pagesize']?$shopset['pagesize']:5;
			$m=M('Shop_goods');
			$goodslist=$m->where('cid='.$cid.' and status=1')->page($page,$pagesize)->order('sorts desc')->select();
			if($goodslist){
				foreach($goodslist as $k=>$v){
					$ta=$this->getPic($goodslist[$k]['pic']);
					$goodslist[$k]['imgurl']=$ta['imgurl'];
	...
	public function goods(){
		...
		//绑定登陆跳转地址
		/*$backurl=base64_encode(U('Wap/Shop/goods',array('id'=>$id)));
	...
	public function basket(){
		$sid=$_SESSION['WAP']['vip']['sid']<>''?$_SESSION['WAP']['vip']['sid']:$this->diemsg(0, '缺少SID参数');//sid可以为0
//		$lasturl=I('lasturl')?I('lasturl'):$this->redirect(U('wap/shop/index'));
		$basketlasturl=base64_decode($lasturl);
		$basketurl=U('Wap/Shop/basket',array('sid'=>$sid,'lasturl'=>$lasturl));		
		...
		$this->assign('cache',$cache);
		$this->assign('totalprice',round($totalprice ,2));
		...
		if(IS_POST){
			...
			if($re){
				...
				if(FALSE !== $old){
					...
					$this->redirect(U('Wap/Shop/pay/',array('sid'=>$data['sid'],'orderid'=>$re)));
				}else{
...
//					$rlog=$mlog->add($dlog);
					//清空购物车
					$rbask=M('Shop_basket')->where(array('sid'=>0,'vipid'=>$data['vipid']))->delete();
					//$this->success('订单创建成功，转向支付界面!',U('Wap/Fxshop/pay/',array('sid'=>$data['sid'],'orderid'=>$re)));
					//转向聚友杀首页
					$this->redirect(U('Wap/Jys/index/',array('sid'=>0,'orderid'=>$re)));
				}else{
					$old=$morder->delete($re);
					$this->error('订单生成失败！请重新尝试！');
				}
	...
	public function orderTuihuo(){
		...
		$sid=$_SESSION['WAP']['vip']['sid']<>''?$_SESSION['WAP']['vip']['sid']:$this->diemsg(0, '缺少SID参数');
		$orderid=I('orderid')<>''?I('orderid'):$this->diemsg(0, '缺少ORDERID参数');
		$bkurl=U('Wap/Shop/orderTuihuo',array('sid'=>$sid,'orderid'=>$orderid));		
		$backurl=base64_encode($bkurl);
		$loginurl=U('Wap/Vip/login',array('backurl'=>$backurl));
		$re=$this->checkLogin($backurl);
		//已登陆
	...
	public function pay(){
		...
		switch($paytype){
					...
					if($re){
						...
						if(FALSE !== $rod){
							...
							if($order['iscut'] && !$order['isgroup']){
								...
								$this->success('余额付款成功！',U('Wap/Shop/orderList',array('sid'=>$sid,'type'=>'2')));
							}
			...
			case 'wxpay':
				...
				$this->redirect(U('/Home/Wxpay/pay',array('oid'=>$order['oid'],'mode'=>$mode)));
			break;
			default:
				$this->error('支付方式未知！');
				break;
	...
	public function label()
	{
		...
		$wap['isgroup']='0';
		$goodslist=$m->where($wap)->order('sorts desc')->select();
//		$next=$m->where('lid='.$id)->page(2,$pagesize)->order('id desc')->select();
//		if($next){
//			$this->assign('nextpage',2);
//		}
		foreach($goodslist as $k=>$v){
	...
	public function labelMore(){
		if(IS_AJAX){						
			...
			$pagesize=$shopset['pagesize']?$shopset['pagesize']:5;
			$m=M('Shop_goods');
			$goodslist=$m->where('lid='.$lid.' and status=1')->page($page,$pagesize)->order('id desc')->select();
			if($goodslist){
				foreach($goodslist as $k=>$v){
					$ta=$this->getPic($goodslist[$k]['pic']);
					$goodslist[$k]['imgurl']=$ta['imgurl'];
	...
	//微信远程登录
	public function wclogin() {
		//这里不做任何事，由基础类托管，不可删除！
		//处理微信网页登录逻辑
		//删除之前的SESSION
		$vipid=$_SESSION['WAP']['vipid'];
		$oauuid=$_SESSION['oauuid'];
		if(!$vipid){
			session(null);
			$this -> diemsg(0, "未获取会员数据，请重新扫描登录！");
		}
		if(!$oauuid){
			session(null);
			$this -> diemsg(0, "缺少参数，请重新扫描登录！");
		}
		$re=M('wclogin')->where("vipid=".$vipid." or ssid='".$oauuid."'")->delete();
		if ($oauuid) {
			$dt['ssid'] = $oauuid;
			$dt['vipid'] = $vipid;
			$dt['ctime'] = time();
			$rlogin = M('wclogin') -> add($dt);
			if ($rlogin) {
				session(null);
				$this -> diemsg(1, "登录成功！若网站未刷新状态，请手动刷新网站！");
			}else{
				session(null);
				$this -> diemsg(1, "登录失败！请重新扫描登录！");
			}
			
		} else {
			unset($_SESSION['oauuid']);
			$this -> diemsg(0, "缺少参数，请重新扫描登录！");
		}
	}
	/**
	...
	private function _getpostage($prid,$heavy){
		...
		$area['heavylist'] =substr($area['heavylist'], 0,strlen($area['heavylist'])-1);
		$area['moneylist'] =substr($area['moneylist'], 0,strlen($area['moneylist'])-1);
		//计算邮费价格
		...
		if($isin){
			if($heavy<1){
				...
				$yf=$yf+round($heavy-end($heavylist))*end($moneylist);
			}			
		}
		return $yf;
	}
