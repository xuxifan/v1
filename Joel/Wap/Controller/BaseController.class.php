...
//		die();
		...
		$_SESSION['oasid']=intval($_GET['sid'])?intval($_GET['sid']):0;
		...
		if (!$_SESSION['oatype']) {
			...
			$_SESSION['oaurl'] = 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
			//绑定微信登录
			$_SESSION['oauuid'] = $_GET['uuid'] ? $_GET['uuid'] : 0;
		}
		//鉴权判定流程
		//判断是否进行base鉴权
		if(!$_SESSION['sqmode']){
...
//				$_SESSION['sqopenid']='o8osiwtNm_DaFZ90btwg3Oy6Lxlk';
		...
		if($_SESSION['sqmode'] && $_SESSION['sqopenid']){
			...
			if(!$vip){
				//邦定来路
				$_SESSION['oacname']=strtolower(CONTROLLER_NAME);
				//绑定控制器
				$_SESSION['oaaname']=strtolower(ACTION_NAME);
			...
			self::$WAP['sid']=$_SESSION['WAP']['sid']=$vip['sid'];
			//追入分销逻辑
			if($vip['sid']){
				//重新覆盖oasid
				...
				if(strtolower(CONTROLLER_NAME) == 'fxshop'){
				   ...
				   if(strtolower(CONTROLLER_NAME) == 'fxshop'){
					...
					if(strtolower(CONTROLLER_NAME) == 'shop'){
						...
						if(strtolower(ACTION_NAME)=='index'){
							$footlasturl=base64_encode(U('Wap/Shop/index'));
							//临时处理不能分享的问题
							$this->assign('allowshare',1);
						}
						if(strtolower(ACTION_NAME)=='goods'){
							$footlasturl=base64_encode(U('Wap/Shop/goods',array('id'=>$_GET['id'])));
							//临时处理不能分享的问题
							$this->assign('allowshare',1);
						}
			...
			unset($_SESSION['oasid']);
			unset($_SESSION['oaurl']);
			unset($_SESSION['oaaname']);
			unset($_SESSION['oacname']);
			
			
		}else{
			//$_tourl=$_SESSION['oaurl'];
			session(null);
...
//			if(strtolower(ACTION_NAME)=='index'){
//				$rurl=U('Wap/Shop/index',array('ppid'=>self::$WAP['vipid'],'type'=>I('type')));
//				$this->redirect($rurl);	
//			}
//			if(strtolower(ACTION_NAME)=='goods'){
//				$rurl=U('Wap/Shop/goods',array('id'=>$_GET['id'],'ppid'=>self::$WAP['vipid']));
//				$this->redirect($rurl);	
//			}			
//		}
//		if(strtolower(CONTROLLER_NAME) == 'fxshop' && $_GET['ppid']<>self::$WAP['vipid']){
//			if(strtolower(ACTION_NAME)=='index'){
//				$rurl=U('Wap/Fxshop/index',array('ppid'=>self::$WAP['vipid'],'type'=>I('type')));
//				$this->redirect($rurl);	
//			}
//			if(strtolower(ACTION_NAME)=='goods'){
//				$rurl=U('Wap/Fxshop/goods',array('id'=>$_GET['id'],'ppid'=>self::$WAP['vipid']));
//				$this->redirect($rurl);	
//			}			
//		}
		//初始化全局会员配置
	...
	public function getPic($id){
		...
		if($list){
			if($list['savename']){
				...
				$list['imgurl']="/upload/img/".$list['name'];
			}
		}
		return $list?$list:"";
	}
