...
class ZjdController extends BaseController {
	// 首页的刷新条目数
	...
	public function index() {
		...
		if($cjdata['isform']){
			...
			if(M("Zjd_acform_data")->where($map_acform)->find()){
				// 填写过信息
				$cjdata['hastx']=1;
			}else{
				$cjdata['hastx']=0;
		...
		// 免费次数用完后的抽奖消耗物品$_SESSION['sqopenid']) {
			$this->assign('zj',$zj);
	...
	protected function getNum($dzp){
		...
		$all_times=M("Zjd_log")->where($Dzp_log_map)->count();
		// 如果总抽奖次数有限制
		if($dzp['cjtimes']){
		...
		if($dzp['daytimes']){
			...
			$day_times=M("Zjd_log")->where($day_map)->count();
			// 超出的每日的抽奖次数
				...
				if($getnum['info']){
					...
					if($fx['status']){
						...
						if($fx['times']){
							...
							$re=M("Zjd_vip_fx")->where($vip_fx_map)->setInc("usetimes");
							$fx['times']--;
							if(!$re){
								$info['status']=1;
								$info['info']="出错啦，请刷新重试~";
	...
	protected function countShare($dzp){
		...
		if($dzp['isfx']){
			...
			$vip_fx=M("Zjd_vip_fx")->where($map)->find();
			if($vip_fx){
				// 已经分享过
	...
	protected function getPrize($info,$dzpid){
		...
		if($res['id']){
			...
			// dump($prize_goods['id']);
			// 查询中奖人数
			$day_zj=M('Zjd_zj')->where($zj_map)->count();
	 ...
	 ******************/
	protected function getRand($dzpid=1){
		// 随机抽奖的产生
		$map['id']=$dzpid;//I('id');
		// 获取项目的信息
		$dzp=M("Zjd")->where($map)->find();
		...
		$map['dzpid']=$map['id'];
		unset($map['id']);
		$prize=M("Zjd_prize")->where($map)->order('level')->select();
		// 现在的时间戳
		...
		if(!$haveday){
			...
			// 是否已经记录了今天的预计人数
			$yjrs=M('Zjd_zj_log')->where($zj_log_map)->find();
			if(!$yjrs){
				$zj_log_map['yjrs']=$yjrs['yjrs']=$dzp['yjrs'];
				$zj_log_map['ctime']=time();
				M('Zjd_zj_log')->add($zj_log_map);
			}
			$yjrs=$yjrs['yjrs'];
			// 总的中奖概率
			...
			// 获得昨天的预计人数
			$yjrs=M('Zjd_zj_log')->where($zj_log_map)->getField('yjrs');
			// 如果昨天没人抽奖
			...
			$log_map['dzpid']=$dzpid;
			// dump($log_map);
			$truenum=M("Zjd_log")->where($log_map)->count();
			// 实际的中奖人数/中奖奖品数量(前一天的)
			$zjnum=M("Zjd_zj")->where($log_map)->count();
			// dump($log_map);
