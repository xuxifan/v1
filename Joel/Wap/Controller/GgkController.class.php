...
class GgkController extends BaseController {
	...
	public function index() {
		...
		// 遍历砸金蛋的图片
		$cjdata['cjimg']=$this->getPic($dzp['dzpimg']);
		// 遍历砸金蛋的图片
		// 1-15  改变兑奖的时间戳
		...
		if($cjdata['isform']){
			...
			if(M("Ggk_acform_data")->where($map_acform)->find()){
				// 填写过信息
				$cjdata['hastx']=1;
			}else{
				$cjdata['hastx']=0;
	...
	protected function getNum($dzp){
		...
		$all_times=M("Ggk_log")->where($Dzp_log_map)->count();
		// 如果总抽奖次数有限制
		if($dzp['cjtimes']){
		...
		if($dzp['daytimes']){
			...
			$day_times=M("Ggk_log")->where($day_map)->count();
			// 超出的每日的抽奖次数
	...
	public function saveCjResult(){                                   
		if(IS_AJAX){
				...
				if($getnum['info']){
					...
					if($fx['status']){
						...
						if($fx['times']){
							...
							$re=M("Ggk_vip_fx")->where($vip_fx_map)->setInc("usetimes");
							$fx['times']--;
							if(!$re){
								$info['status']=1;
								$info['info']="出错啦，请刷新重试~";
			...
			// 判断是否开启可控
			if($dzp['ifcontrol']){
				$resreturn=$this->getRandIsControl(I("id"));
			}else{
				$resreturn=$this->getRand(I("id"));
			}
			$res['id']=$resreturn['id'];
			$res['sn']=$resreturn['sn'];
	...
	protected function countShare($dzp){
		...
		if($dzp['isfx']){
			...
			$vip_fx=M("Ggk_vip_fx")->where($map)->find();
			if($vip_fx){
				// 已经分享过
	...
	protected function getPrize($info,$dzpid){
		...
		if($res['id']){
			...
			// dump($prize_goods['id']);
			// 查询中奖人数
			$day_zj=M('Ggk_zj')->where($zj_map)->count();
	...
	protected function getRand($dzpid){
		...
		$map['dzpid']=$map['id'];
		unset($map['id']);
		$prize=M("Ggk_prize")->where($map)->order('level')->select();
		// 现在的时间戳
		...
		if(!$haveday){
			...
			// 是否已经记录了今天的预计人数
			$yjrs=M('Ggk_zj_log')->where($zj_log_map)->find();
			if(!$yjrs){
				$zj_log_map['yjrs']=$yjrs['yjrs']=$dzp['yjrs'];
				$zj_log_map['ctime']=time();
				M('Ggk_zj_log')->add($zj_log_map);
			}
			$yjrs=$yjrs['yjrs'];
			// 总的中奖概率
			...
			// 获得昨天的预计人数
			$yjrs=M('Ggk_zj_log')->where($zj_log_map)->getField('yjrs');
			// 如果昨天没人抽奖
			...
			$log_map['dzpid']=$dzpid;
			// dump($log_map);
			$truenum=M("Ggk_log")->where($log_map)->count();
			// 实际的中奖人数/中奖奖品数量(前一天的)
			$zjnum=M("Ggk_zj")->where($log_map)->count();
			// dump($log_map);
	...
	public function getdata(){
		...
		$psize = self::$psizee;
		// $map =array('lid'=>$id,'openprize'=>1,'type'=>3);
		$map =array('dzpid'=>$id);
