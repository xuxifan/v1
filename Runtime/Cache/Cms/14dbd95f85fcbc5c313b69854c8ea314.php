		...
		<div class="widget">
			...
			<div class="widget-body">
				<div class="table-toolbar">
					<a href="<?php echo U('Cms/Vip/cardSet/');?>" class="btn btn-primary" data-loader="Joel-loader" data-loadername="卡券设置">
					...
					<div class="pull-right">
						<form id="Joel-search">
							...
							<a href="<?php echo U('Cms/Vip/card',array('type'=>$type));?>" class="btn btn-success" data-loader="Joel-loader" data-loadername="卡券列表" data-search="Joel-search">
				...
				<table id="Joel-table" class="table table-bordered table-hover">
					<thead class="bordered-darkorange">
						<tr role="row">
							...
							<?php if(in_array(($pwd), is_array($useroath)?$useroath:explode(',',$useroath))): ?><th width="100px">卡密</th><?php endif; ?>	
					...
					<tbody>
						<?php if(is_array($cache)): $i = 0; $__LIST__ = $cache;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id="item<?php echo ($vo["id"]); ?>">
								...
								<?php if(in_array(($pwd), is_array($useroath)?$useroath:explode(',',$useroath))): ?><td class=" "><?php echo ($vo["cardpwd"]); ?></td><?php endif; ?>
								...
								<td class="center "><a href="<?php echo U('Cms/vip/card/');?>" class="btn btn-danger btn-xs" data-type = "del" data-ajax="<?php echo U('Cms/vip/cardDel/',array('id'=>$vo['id']));?>" ><i class="fa fa-trash-o"></i> 删除</a></eq></td>
...
<script type="text/javascript">
	...
	$("#sendCard").on('click', function () {
		...
		if(checks.length==0){
			$.Joel.alert('danger','请选择要发送的卡券！');
			return false;
		}
		if(checks.length>1){
			$.Joel.alert('danger','单次仅能发送一张卡券！');
			return false;
		}
		var cardid=checks.val();
		var cardstatus=$('#status'+cardid).text();
		if(cardstatus!='生成'){
			$.Joel.alert('danger','此卡券不能发放！');
			return false;
		}
		if($('#type'+cardid).text()=='充值卡'){
			$.Joel.alert('danger','充值卡暂不支持线上发放！');
			return false;
		}
        ...
        bootbox.prompt(carddetail, function (result) {
            if (result != null) {
                ...
                var tourl="<?php echo U('Cms/vip/sendCard');?>";
            	$.Joel.ajax('post',tourl,data,function(){$('#refresh-toggler').trigger('click');});
            }
        });
	...
	$('#Joel-delall').on('click',function(){
		...
		var toajax="<?php echo U('Cms/Vip/cardDel');?>"+"/id/"+chk;
		var funok=function(){
			var callok=function(){
				//成功删除后刷新
	...
	$('#exportCard').on('click',function(){
		...
		window.open("<?php echo U('Cms/Vip/cardExport');?>/type/<?php echo ($type); ?>/id/"+chk);
	})
</script>
<!--/全选特效封装-->